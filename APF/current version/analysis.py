...
#BEFORE RUNNING:
#1. CHANGE LINE 132 TO 2040
#2. DECIDE ON 3 OR 5 LAYER NN ON LINE 29. NN FOR 3 LAYER, NN5 FOR 5 LAYER
...
#establish a network using the nn class. OR USE nn5 for 5 layer network
APF_nn=nn5(400,200,200,200,2) #5 arguments for 5 layer nn, 3 arguments for 3 layer nn
...
def train_nn(sample_x_start=0,sample_y_start=170):
  ...
  ####### #will do get_sample, then process_sample for each (sample_x,sample_y) here. Will then apply NN to processed sample
            ...
            APF_nn.backpropogate([-1,1],train_input,0.5)#backpropogate sample. Output target is 0. learning rate assumed to be 0.5

            #feedforward a simulated detection
            #GENERATE SIMULATED SAMPLES
            a=sample_maker.generate_narrowband_1().flatten()
            b=sample_maker.generate_narrowband_2().flatten()
            c=sample_maker.generate_narrowband_1CR().flatten()
            d=sample_maker.generate_narrowband_2CR().flatten()

            #choose and feedforward one of the simulated detections
            choice=random.randint(0,3)

            if choice==0:
                APF_nn.feedforward(a)#feed the sample through the nn
                APF_nn.backpropogate([1,-1],a,0.5)#backpropogate sample. Output target is 1. learning rate assumed to be 0.5

            if choice==1:
                APF_nn.feedforward(b)#feed the sample through the nn
                APF_nn.backpropogate([1,-1],b,0.5)#backpropogate sample.

            if choice==2:
                APF_nn.feedforward(c)#feed the sample through the nn
                APF_nn.backpropogate([1,-1],c,0.5)#backpropogate sample.

            if choice==3:
                APF_nn.feedforward(d)#feed the sample through the nn
                APF_nn.backpropogate([1,-1],d,0.5)#backpropogate sample.
...
def run(sample_x_start=0,sample_y_start=170):
  ...
  ####### #will do get_sample, then process_sample for each (sample_x,sample_y) here. Will then apply NN to processed sample
            #sample_list.append([sample_x,sample_y])
            sample_x+=1#increment by 1 - moving sample across (could change to 20 but would need to make more complex examples to refer against.
            sample_y=sample_y-3+sample_sweep(6,sample_x,(sample_y-3))#Sweeps 3 pixels each direction for each step across
